<template>
    <DocumentOperate v-model:entityRecognitionLoading="entityRecognitionLoading" v-model:entityList="entityList"
        @ai-pre-request="handleAiRequest" @anonymous-processing="docNameEntityRecognition" />
    <div class="flex-1 hide-scrobar flex flex-col items-stretch overflow-y-scroll mb-3" ref="scrollContainer"
        style="scrollbar-width: none; -ms-overflow-style: none">
        <ChatMessage v-for="m in messages" :message="m" :export="exportMode"
            @text-selected="(text) => $emit('textSelected', text)" @deleted-message="handleDeleteMessage"
            @switch-export="(id) => switchExport(id)" @update-response-success="updateMessege">
        </ChatMessage>
        <div ref="viewAnchor" />
    </div>
    ...
    <div v-if="!exportMode" class="self-stretch flex mb-3 justify-between">
        <el-autocomplete class="flex-1 inline-input" v-model="prompt" :fetch-suggestions="querySearch"
            :trigger-on-focus="false" clearable placeholder="输入 / 选择或者直接提问" :disabled="entityRecognitionLoading">
        </el-autocomplete>
        <el-button @click="docAnalyze" :loading="loading" class="ml-3" type="success"
            :disabled="entityRecognitionLoading" :icon="Promotion" />
    </div>
    <div v-else class="self-stretch flex mb-3 justify-between items-center">
        <el-checkbox class="self-center" v-model="checkAll" :indeterminate="isIndeterminate"
...
<script lang="ts" setup>
    ...
    const getEntityList = async () => {
        // const { data } = await axios.post(`/api/document/${props.document.doc_id}/entity`)
        console.log(props.document.doc_id, '===')
    }

    let intervalId
    onMounted(async () => {
        console.log(props.document.doc_id, '===document.doc_id')
