<template>
  <div class="flex justify-between">
    <el-input v-model="filterText" style="width: 180px" placeholder="输入文件名或标签名过滤" />
...
nextTick(() => [setCheckKeys(checkedKeys.value)])
...
const filterNode = (value: string, data: Tree) => {
  ...
  return (
    data.text.includes(value) ||
    (data.dynamicTags && data.dynamicTags.some((item) => item.indexOf(value) > -1))
  )
}
...
const handleDocDelete = (data: Tree) => {
  ...
  axios
    .post(`/api/${props.book.identify}/delete`, formData) // 替换为实际的API地址
    .then((response) => {
...
//   left: 0,
// })
const handleNodeContextmenu = (_event, node, data) => {
...
const handleEditDocNameConfirm = (key, data) => {
  ...
  formData.append('doc_id', data.id)
  axios
    .post(`/api/${props.book.identify}/create`, formData)
    .then((response) => {
      if (response.data.errcode == 0) {
        ElMessage.success(response.data.message)
