    ...
    <el-container class="flex-1">
        ...
        <el-container>
            <el-aside padding="20px">
                <div class="flex flex-col" v-loading="loading" width="200px">
                    <div class="flex justify-between" v-for="f in functionsPage.List" :key="f.id">
                        <el-button
                            size="large"
                            :key="f.id"
                    ...
                    <el-button disable="loading" type="primary" @click="handleCreate">新建</el-button>
                </div>
            </el-aside>
            <el-main>
                <div v-loading="loading" class="flex flex-row">
...
onMounted(async () => {
    ElMessage.warning("正在获取可用模型和能力，有点慢，请稍候")
    loading.value = true
    ...
    if (response.data.errcode !== 0) {
        ...
        llmModels.value = response.data.data
    }
    response = await axios.get("/api/ai/function")
    if (response.data.errcode !== 0) {
        ElMessage.warning(response.data.message)
    } else {
...
const listModels = async () => {
    console.log(llmModels.value)
    options.value = llmModels.value
}
...
const handleCreate = async () => {
    ...
    const f = {
        template_name: "自定义能力",
        template: "帮我总结这个文档",
        model_name: llmModels.value[0],
        score_threshold: 1.0,
        top_k: 3,
        temperature: 0.1,
    }
    const response = await axios.post('/api/ai/function', f)
    const { errcode, message } = response.data
    if (errcode === 0) {
